...
def build_sampler(batch_size, input_size_x, input_size_y, n_classes=1, n_samples=1, sample_mode='st', n_channels=4,
    ...
    onehot_background_dense = Embedding(n_classes, input_size_x * input_size_y * n_channels,
                                        embeddings_initializer='zeros', name=model_prefix + 'background_dense')

    # Initialize template and mask matrices
    onehot_template_dense = Embedding(n_classes, input_size_x * input_size_y * n_channels,
                                      embeddings_initializer='zeros', name=model_prefix + 'template_dense')
    onehot_mask_dense = Embedding(n_classes, input_size_x * input_size_y * n_channels, embeddings_initializer='ones',
...
def make_resblock(n_channels=64, window_size=8, dilation_rate=1, group_ix=0, layer_ix=0, drop_rate=0.0,
    ...
    if norm_mode == 'instance':
        batch_norm_0 = InstanceNormalization(
            name='scrambler_resblock_' + str(group_ix) + '_' + str(layer_ix) + '_norm_0')
    ...
    conv_0 = Conv2D(n_channels, window_size, dilation_rate=dilation_rate, strides=(1, 1), padding='same',
                    activation='linear', kernel_initializer='glorot_normal',
                    name='scrambler_resblock_' + str(group_ix) + '_' + str(layer_ix) + '_conv_0')

    batch_norm_1 = lambda x: x
    if norm_mode == 'instance':
        batch_norm_1 = InstanceNormalization(
            name='scrambler_resblock_' + str(group_ix) + '_' + str(layer_ix) + '_norm_1')
    elif norm_mode == 'batch':
    ...
    conv_1 = Conv2D(n_channels, window_size, dilation_rate=dilation_rate, strides=(1, 1), padding='same',
                    activation='linear', kernel_initializer='glorot_normal',
                    name='scrambler_resblock_' + str(group_ix) + '_' + str(layer_ix) + '_conv_1')
...
def load_scrambler_network(input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4, n_spatial_dims=1,
    ...
    conv_0 = Conv2D(n_channels, (1, 1), strides=(1, 1), padding='same', activation='linear',
                    kernel_initializer='glorot_normal', name='scrambler_conv_0')

    label_concat = None
    if use_label_input:
        label_concat = Lambda(lambda x: K.concatenate([x[0],
                                                       K.tile(K.expand_dims(K.expand_dims(x[1], axis=-1), axis=-1),
                                                              (1, K.shape(x[0])[1], K.shape(x[0])[2], 1))], axis=-1))
    ...
    if mask_dropout:
        if len(mask_drop_scales) <= 1:
            mask_drop = Lambda(
                lambda x: mask_dropout_single_scale(x, drop_scale=mask_drop_scales[0], min_drop_rate=mask_min_drop_rate,
                                                    max_drop_rate=mask_max_drop_rate),
                output_shape=(1, input_size_y, 1) if n_spatial_dims == 1 else (input_size_x, input_size_y, 1),
                name='scrambler_mask_drop')
        else:
            mask_drop = Lambda(
                lambda x: mask_dropout_multi_scale(x, drop_scales=mask_drop_scales, min_drop_rate=mask_min_drop_rate,
                                                   max_drop_rate=mask_max_drop_rate),
                output_shape=(1, input_size_y, 1) if n_spatial_dims == 1 else (input_size_x, input_size_y, 1),
                name='scrambler_mask_drop')
    ...
    for group_ix in range(n_groups):
        ...
        skip_convs.append(Conv2D(n_channels, (1, 1), strides=(1, 1), padding='same', activation='linear',
                                 kernel_initializer='glorot_normal', name='scrambler_skip_conv_' + str(group_ix)))

        resblocks = []
        for layer_ix in range(n_resblocks_per_group):
            resblocks.append(make_resblock(n_channels=n_channels,
                                           window_size=(1, window_size) if n_spatial_dims == 1 else (
                                           window_size, window_size), dilation_rate=dilation_rates[group_ix],
                                           group_ix=group_ix, layer_ix=layer_ix, drop_rate=drop_rate,
                                           norm_mode=norm_mode))

        resblock_groups.append(resblocks)

    last_block_conv = Conv2D(n_channels, (1, 1), strides=(1, 1), padding='same', activation='linear',
                             kernel_initializer='glorot_normal', name='scrambler_last_block_conv')
...
def load_finetuning_model(batch_size, input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4,
                          n_spatial_dims=1, mask_smoothing=False, smooth_window_size=None, norm_mode='instance',
                          max_score_clip=4.):
    ...
    mask_dense = Embedding(batch_size, input_size_x * input_size_y, embeddings_initializer='glorot_normal',
                           name='ft_scrambler_mask_dense')

    mask_reshape = Reshape((input_size_x, input_size_y, 1))

    mask_conv, mask_norm = None, None
    if norm_mode is not None and norm_mode == 'conv':
        mask_conv = Conv2D(1, (1, 1), strides=(1, 1), padding='same', activation='linear',
                           kernel_initializer='glorot_normal', name='ft_scrambler_mask_conv')
    ...
    if mask_smoothing:
        smooth_conv = Conv2D(1, (1, smooth_window_size) if n_spatial_dims == 1 else (
        smooth_window_size, smooth_window_size), strides=(1, 1), use_bias=False, padding='same', activation='linear',
                             kernel_initializer='ones', name='ft_scrambler_smooth_conv')
    ...
    if scrambler_mode == 'occlusion':
        scale_logits = Lambda(lambda x: x[1] / K.maximum(K.tile(x[0], (1, 1, 1, n_out_channels)), K.epsilon()),
                              name='ft_scrambler_logit_scale')

    clip_scores = Lambda(lambda x, max_score_clip=max_score_clip: K.relu(K.clip(x[1], 0., max_score_clip) - x[0]),
                         name='ft_scrambler_clip_scores')
...
def load_optimization_model(batch_size, input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4,
                            n_spatial_dims=1, mask_smoothing=False, smooth_window_size=None, norm_mode='instance'):
    # seed_input = Lambda(lambda x: K.zeros((K.shape(x)[0], 1), dtype=tf.int32))
    seed_input = Lambda(lambda x: K.constant(np.arange(batch_size), dtype=tf.int32))

    mask_dense = Embedding(batch_size, input_size_x * input_size_y, embeddings_initializer='glorot_normal',
                           name='ot_scrambler_mask_dense')
    ...
    if norm_mode is not None and norm_mode == 'conv':
        mask_conv = Conv2D(1, (1, 1), strides=(1, 1), padding='same', activation='linear',
                           kernel_initializer='glorot_normal', name='ot_scrambler_mask_conv')
    elif norm_mode is not None and norm_mode == 'instance':
    ...
    if mask_smoothing:
        smooth_conv = Conv2D(1, (1, smooth_window_size) if n_spatial_dims == 1 else (
        smooth_window_size, smooth_window_size), strides=(1, 1), use_bias=False, padding='same', activation='linear',
                             kernel_initializer='ones', name='ot_scrambler_smooth_conv')
...
def get_sigmoid_kl_divergence():
    def _kl_divergence(y_true, y_pred):
        ...
        return K.mean(y_true * K.log(y_true / y_pred) + (1.0 - y_true) * K.log((1.0 - y_true) / (1.0 - y_pred)),
                      axis=-1)
...
def get_symmetric_sigmoid_kl_divergence():
    def _kl_divergence(y_true, y_pred):
        ...
        left_mean_kl = K.mean(y_true * K.log(y_true / y_pred) + (1.0 - y_true) * K.log((1.0 - y_true) / (1.0 - y_pred)),
                              axis=-1)
        right_mean_kl = K.mean(
            y_pred * K.log(y_pred / y_true) + (1.0 - y_pred) * K.log((1.0 - y_pred) / (1.0 - y_true)), axis=-1)
...
class ScramblerMonitor(Callback):
    def __init__(self, scrambler_model, loss_model, loss_tensors, input_tensors, n_inputs=1, track_mode='batch',
                 batch_freq_dict=None, batch_size=32):
...
class Scrambler:
    ...
    def __init__(self, n_inputs=1, n_classes=1, multi_input_mode='siamese', scrambler_mode='inclusion', input_size_x=1,
                 ...
                 network_config={'n_groups': 1, 'n_resblocks_per_group': 4, 'n_channels': 32, 'window_size': 8,
                                 ...
                                 'mask_drop_scales': [1, 5], 'mask_min_drop_rate': 0.0, 'mask_max_drop_rate': 0.5,
                                 'label_input': False}):
        ...
        for input_ix in range(self.n_inputs):
            ...
            scrambler_inputs.append(Input(shape=(self.input_size_x, self.input_size_y, self.n_out_channels),
                                          name='scrambler_input_' + str(input_ix)))
            if self.mask_dropout:
                scrambler_drops.append(
                    Input(shape=(self.input_size_x, self.input_size_y, 1), name='scrambler_drop_' + str(input_ix)))
            else:
        ...
        for input_ix in range(self.n_inputs):
            ...
            pwm_logit, pwm, sampled_pwm, _, sampled_mask = sampler(scrambler_classes[input_ix],
                                                                   scrambled_logits[input_ix])
        ...
        scrambler_model = Model(
            scrambler_classes + scrambler_inputs + (scrambler_drops if scrambler_drops[0] is not None else []) + (
                [scrambler_label] if scrambler_label is not None else []),
        ...
        if self.mask_smoothing:
            scrambler_model.get_layer("scrambler_smooth_conv").set_weights([
                np.reshape(np.array(mask_smoothing_conv_weight),
                           (1, self.mask_smoothing_window_size, 1, 1) if self.input_size_x == 1 else (
                           self.mask_smoothing_window_size, self.mask_smoothing_window_size, 1, 1))
            ])
    ...
    def train(self, predictor, x_train, y_train, x_test, y_test, n_epochs, extra_input_train=None,
        ...
        for input_ix in range(self.n_inputs):
            ...
            scrambler_inputs.append(Input(shape=(self.input_size_x, self.input_size_y, self.n_out_channels),
                                          name='t_scrambler_input_' + str(input_ix)))
            if self.mask_dropout:
                scrambler_drops.append(
                    Input(shape=(self.input_size_x, self.input_size_y, 1), name='t_scrambler_drop_' + str(input_ix)))
            else:
                scrambler_drops.append(None)
        ...
        for extra_in_ix, extra_in in enumerate(extra_input_train):
            scrambler_extra_inputs.append(
                Input(shape=tuple(list(extra_in.shape)[1:]), name='t_scrambler_extra_' + str(extra_in_ix)))
        ...
        for input_ix in range(self.n_inputs):
            ...
            pwm_logit, pwm, sampled_pwm, pwm_mask, sampled_mask = self.sampler(scrambler_classes[input_ix],
                                                                               scrambled_logits[input_ix])
        ...
        checkpointer = ModelCheckpoint(filepath="saved_models/temp.h5", verbose=1, save_best_only=True, monitor='loss',
        ...
        drop_train = [
            np.ones((x_train[0].shape[0], x_train[0].shape[1], x_train[0].shape[2], 1)) for input_ix in
            range(self.n_inputs)
        ] if self.mask_dropout else []
        drop_test = [
            np.ones((x_test[0].shape[0], x_test[0].shape[1], x_test[0].shape[2], 1)) for input_ix in
            range(self.n_inputs)
        ] if self.mask_dropout else []
        ...
        if monitor_test_indices is not None:
            ...
            monitor = ScramblerMonitor(self.scrambler_model, self.loss_model, monitor_loss_tensors, monitor_tensors,
                                       n_inputs=self.n_inputs, track_mode='batch',
                                       batch_freq_dict=monitor_batch_freq_dict, batch_size=self.batch_size)
        ...
        loss_model.load_weights("saved_models/temp.h5")
    ...
    def finetune(self, predictor, x, y, batch_size, n_iters, drop=None, extra_input=None, group=None,
                 norm_mode='instance', max_score_clip=4., adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9,
                 nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None,
                 reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1.):
        ...
        return self._optimize(predictor, finetuner, [pretrained_scores], x, y, batch_size, n_iters, 'finetune',
                              drop=drop, extra_input=extra_input, group=group, adam_lr=adam_lr, adam_beta_1=adam_beta_1,
                              adam_beta_2=adam_beta_2, nll_mode=nll_mode, predictor_task=predictor_task,
                              custom_loss_func=custom_loss_func, reference=reference, entropy_mode=entropy_mode,
                              entropy_bits=entropy_bits, entropy_weight=entropy_weight)

    def optimize(self, predictor, x, y, batch_size, n_iters, drop=None, extra_input=None, group=None,
                 norm_mode='instance', adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9, nll_mode='reconstruction',
                 predictor_task='classification', custom_loss_func=None, reference='predictor', entropy_mode='target',
                 entropy_bits=0., entropy_weight=1.):
        ...
        return self._optimize(predictor, optimizer, [], x, y, batch_size, n_iters, 'optimize', drop=drop,
                              extra_input=extra_input, group=group, adam_lr=adam_lr, adam_beta_1=adam_beta_1,
                              adam_beta_2=adam_beta_2, nll_mode=nll_mode, predictor_task=predictor_task,
                              custom_loss_func=custom_loss_func, reference=reference, entropy_mode=entropy_mode,
                              entropy_bits=entropy_bits, entropy_weight=entropy_weight)

    def _optimize(self, predictor, finetuner, pretrained_scores, x, y, batch_size, n_iters, opt_mode, drop=None,
                  extra_input=None, group=None, adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9,
                  nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None,
                  reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1.):
        ...
        # Load sampler
        finetuning_sampler = build_sampler(batch_size, self.input_size_x, self.input_size_y,
                                           n_classes=len(self.input_templates), n_samples=self.n_samples,
                                           sample_mode=self.sample_mode, n_channels=self.n_out_channels,
                                           model_prefix='ft_')
        ...
        for input_ix in range(self.n_inputs):
            ...
            scrambler_inputs.append(
                Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, self.n_out_channels),
                      name='f_scrambler_input_' + str(input_ix)))
            scrambler_drops.append(Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1),
                                         name='f_scrambler_drop_' + str(input_ix)))
            if opt_mode == 'finetune':
                scrambler_pretrained_scores.append(
                    Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1),
                          name='f_scrambler_pretrained_scores_' + str(input_ix)))
        ...
        if self.multi_input_mode == 'siamese' or self.n_inputs == 1:
            ...
            for input_ix in range(self.n_inputs):
                ...
                if opt_mode == 'finetune':
                    scrambled_logit, importance_score = finetuner(scrambler_inputs[input_ix], scrambler_drops[input_ix],
                                                                  scrambler_pretrained_scores[input_ix])
                else:
                    scrambled_logit, importance_score = finetuner(scrambler_inputs[input_ix], scrambler_drops[input_ix])
            ...
            scrambler_input = scrambler_input_concat(scrambler_inputs)
            scrambler_drop = scrambler_input_concat(scrambler_drops) if self.mask_dropout else None
            scrambler_pretrained_score = scrambler_pretrained_score_concat(
                scrambler_pretrained_scores) if opt_mode == 'finetune' else None

            scrambled_logit, importance_score = None, None
            if opt_mode == 'finetune':
                scrambled_logit, importance_score = finetuner(scrambler_input, scrambler_drop,
                                                              scrambler_pretrained_score)
            else:
                scrambled_logit, importance_score = finetuner(scrambler_input, scrambler_drop)
        ...
        for input_ix in range(self.n_inputs):
            ...
            pwm_logit, pwm, sampled_pwm, _, sampled_mask = finetuning_sampler(scrambler_classes[input_ix],
                                                                              scrambled_logits[input_ix])
        ...
        if self.mask_smoothing:
            finetuning_model.get_layer("ft_scrambler_smooth_conv").set_weights([
                np.reshape(np.array(mask_smoothing_conv_weight),
                           (1, self.mask_smoothing_window_size, 1, 1) if input_size_x == 1 else (
                           self.mask_smoothing_window_size, self.mask_smoothing_window_size, 1, 1))
            ])
        ...
        for input_ix in range(self.n_inputs):
            ...
            scrambler_inputs.append(
                Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, self.n_out_channels),
                      name='ft_scrambler_input_' + str(input_ix)))
            scrambler_drops.append(Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1),
                                         name='ft_scrambler_drop_' + str(input_ix)))
            if opt_mode == 'finetune':
                scrambler_pretrained_scores.append(
                    Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1),
                          name='ft_scrambler_pretrained_scores_' + str(input_ix)))
        ...
        for extra_in_ix, extra_in in enumerate(extra_input):
            scrambler_extra_inputs.append(Input(batch_shape=tuple([batch_size] + list(extra_in.shape)[1:]),
                                                name='ft_scrambler_extra_' + str(extra_in_ix)))
        ...
        for input_ix in range(self.n_inputs):
            ...
            pwm_logit, pwm, sampled_pwm, pwm_mask, sampled_mask = finetuning_sampler(scrambler_classes[input_ix],
                                                                                     scrambled_logits[input_ix])
        ...
        # Define layer to inflate sample axis
        inflate_non_scrambled_prediction = Lambda(lambda x: K.tile(K.expand_dims(x, axis=1), (1, self.n_samples, 1)),
                                                  name='ft_inflate_non_scrambled_prediction')
        inflate_scrambled_prediction = Lambda(lambda x: K.reshape(x, (self.batch_size, self.n_samples, self.n_classes)),
                                              name='ft_inflate_scrambled_prediction')
        ...
        x_mean_dense = Embedding(len(self.input_templates), self.input_size_x * self.input_size_y * self.n_out_channels,
                                 embeddings_initializer='zeros', name='ft_x_mean_dense')
        ...
        nll_loss = Lambda(lambda x: K.reshape(K.sum(scrambler_mode_coeff * nll_loss_func(x[0], x[1]), axis=0), (1,)),
                          name='ft_nll')([
            y_pred_non_scrambled,
            y_pred_scrambled
        ])
        ...
        entropy_loss = None
        if len(entropy_losses) > 1:
            entropy_loss = Lambda(lambda x: K.reshape(K.sum(K.mean(x, axis=-1), axis=0), (1,)), name='ft_entropy')(
                Concatenate(axis=-1)(entropy_losses))
        else:
            entropy_loss = Lambda(lambda x: K.reshape(K.sum(K.mean(x, axis=-1), axis=0), (1,)), name='ft_entropy')(
                entropy_losses[0])
        ...
        # Initialize Templates and Masks
        initialize_templates(finetuning_loss_model, self.input_templates, self.input_backgrounds_log,
                             model_prefix='ft_')
